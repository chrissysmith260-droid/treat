    ...
    <div style={{ 
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
        <input
          ...
          style={{ padding: '8px', borderRadius: '8px', border: '1px solid #ddd', flex: 1, backgroundColor: '#D1E9FF' }}
        />
        <input
        ...
        <select
          ...
          style={{ padding: '8px', borderRadius: '8px', border: '1px solid #ddd', backgroundColor: '#D1E9FF' }}
        >
          <option value="Low">Low</option>
          <option value="Moderate">Moderate</option>
          <option value="Severe">Severe</option>
        </select>
        <button onClick={handleAdd} style={{ background: theme.colors.leafGreen, border: 'none', color: 'white', textShadow: '1px 1px 0 #000, -1px 1px 0 #000, 1px -1px 0 #000, -1px -1px 0 #000', borderRadius: '8px', padding: '8px', cursor: 'pointer' }}>
          <Plus size={20} style={iconOutline} />
        </button>
      </div>
            ...
            {allergies.map((allergy, index) => (
              <tr key={index} style={{ borderBottom: '1px solid #f5f5f5' }}>
                <td style={{ padding: '12px 10px', color: theme.colors.textDark, fontWeight: 'bold' }}>
                  {editingIndex === index ? (
                    <input value={editSubstance} onChange={(e) => setEditSubstance(e.target.value)} style={{ width: '100%', backgroundColor: '#D1E9FF' }} />
                  ) : allergy.substance}
                </td>
                <td style={{ padding: '12px 10px', color: '#777' }}>
                  {editingIndex === index ? (
                    <input value={editReaction} onChange={(e) => setEditReaction(e.target.value)} style={{ width: '100%', backgroundColor: '#D1E9FF' }} />
                  ) : allergy.reaction}
                </td>
                <td style={{ padding: '12px 10px' }}>
                  {editingIndex === index ? (
                    <select value={editSeverity} onChange={(e) => setEditSeverity(e.target.value)} style={{ padding: '8px', borderRadius: '8px', border: '1px solid #ddd', backgroundColor: '#D1E9FF' }}>
                      <option value="Low">Low</option>
                      <option value="Moderate">Moderate</option>
                      <option value="Severe">Severe</option>
                    </select>
                ...
                <td style={{ padding: '12px 10px', textAlign: 'right' }}>
                  {editingIndex === index ? (
                    <>
                      <Check size={18} color={theme.colors.leafGreen} style={{ cursor: 'pointer', marginRight: '10px', ...iconOutline }} onClick={() => handleSaveEdit(index)} />
                      <X size={18} color={theme.colors.errorRed} style={{ cursor: 'pointer', ...iconOutline }} onClick={() => setEditingIndex(null)} />
                    </>
                  ) : (
                    <>
                      <Edit2 size={18} color={theme.colors.accentBlue} style={{ cursor: 'pointer', marginRight: '10px', ...iconOutline }} onClick={() => handleStartEdit(index)} />
                      <Trash2 size={18} color={theme.colors.errorRed} style={{ cursor: 'pointer', ...iconOutline }} onClick={() => handleDelete(index)} />
                    </>
                  )}
                </td>
              </tr>
